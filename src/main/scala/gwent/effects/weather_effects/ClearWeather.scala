...
class ClearWeather extends WeatherEffect {
  override def visitCloseCombatCard(card: CloseCombatCard): Unit = card.exitWeather()
  override def visitRangeCombatCard(card:  RangeCombatCard): Unit = card.exitWeather()
  override def visitSiegeCombatCard(card: SiegeCombatCard): Unit = card.exitWeather()
